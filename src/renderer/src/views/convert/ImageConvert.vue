  ...
  <div class="convert-tool">
    ...
    <div class="convert-tool__upload">
      <el-upload
        ...
        accept=".jpg,.jpeg,.png,.webp,.gif,.bmp,.heic,.avif,.tiff,.tif,.svg,.ico,.apng"
      >
        <div class="upload-drag__placeholder">
          ...
          <p class="upload-drag__text">拖拽图片到此处或点击上传</p>
        </div>
      </el-upload>
      ...
      >
        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#67c23a" stroke-width="1.5">
          <rect x="3" y="3" width="18" height="18" rx="2" />
          <circle cx="8.5" cy="8.5" r="1.5" />
          <polyline points="21 15 16 10 5 21" />
        </svg>
...
const {
  ...
  selectTargetFormat,
  startConvert
} = useFileConvert(imageRules, null, imageTypeExtensions, {
  strictType: false,
  convertFn: (fileObj, targetFmt, onProgress) => {
    onProgress(10)
    return dispatchConvert(fileObj, targetFmt).then((blob) => {
      onProgress(100)
      return blob
    })
  }
...
function onFileChange(uploadFile) {
  ...
  for (const [type, exts] of Object.entries(imageTypeExtensions)) {
...
function onReplaceFile(e) {
  ...
  if (f) {
    ...
    for (const [type, exts] of Object.entries(imageTypeExtensions)) {
      if (exts.includes(ext)) { selectedType.value = type; return }
    }
    selectedType.value = ''
  }
  e.target.value = ''
