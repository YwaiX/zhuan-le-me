    ...
    <div class="convert-tool__upload">
      ...
      >
        <div class="upload-drag__placeholder">
          ...
          <p class="upload-drag__text">拖拽文件到此处或点击上传</p>
        </div>
      </el-upload>
...
// Office → PDF 转换（通过 LibreOffice IPC）
async function convertOffice(fileObj, targetFmt, onProgress) {
  onProgress(10)
  const arrayBuffer = await fileObj.arrayBuffer()
  const data = Array.from(new Uint8Array(arrayBuffer))
  const ext = fileObj.name.split('.').pop()?.toLowerCase() || ''
  const result = await window.api.libreOfficeConvert(data, ext, targetFmt)
  onProgress(100)
...
const {
  ...
  selectTargetFormat,
  startConvert
} = useFileConvert(officeRules, null, officeTypeExtensions, { strictType: false, convertFn: convertOffice })
...
function onFileChange(uploadFile) {
  ...
  for (const [type, exts] of Object.entries(officeTypeExtensions)) {
